...
class Emitter():
    ...
    def emitREOP(self, op, in_, frame):
        ...
        if op == ">":
            if type(in_) is cgen.FloatType:
                # Compare value1 with value2: (>,1), (=,0), (...,-1)
    ...
    *   @param isStatic <code>true</code> if the method is static; <code>false</code> otherwise.
    ...
    def getConst(self, ast):
        ...
        if type(ast) is IntLiteral:
            ...
            return (str(ast.value), cgen.FloatType())
        elif type(ast) is StringLiteral:
            return (ast.value, cgen.StringType())
        elif type(ast) is BooleanLiteral:
            return (str(ast.value), cgen.BoolType())
        elif type(ast) is ArrayLiteral:
            return self.getConst(ast.value[0])
    ...
    *   @param in the type of the local array variable.
    ...
    def emitARRAY(self, in_, frame):
        ...
        if type(in_) is not cgen.ArrayType:
            raise IllegalOperandException(str(in_))

        typ = in_.eleType
        for d in in_.dimen[::-1]:
            typ = cgen.ArrayType(typ, d)

        dimensions = in_.dimen
        result = [self.emitPUSHICONST(dimen, frame) for dimen in dimensions]
        [frame.pop() for dimen in dimensions]
        if len(result) == 1:
            if type(in_) is cgen.StringType:
    ...
    *   @param in the type of the returned expression.
    ...
    def emitRETURN(self, in_, frame):
        ...
        if type(in_) is cgen.IntType or type(in_) is cgen.BoolType or type(in_) is cgen.StringType:
            frame.pop()
